...
class ParsingColor{
  static List<Color> cekColor(String status) {
    if (status == 'high' || status == 'tinggi' || status == 'Menunggu' || status == 'Maintenance' || status == 'Rating' || status  == 'Scheduled') {
      return [AppColor.accentHigh, AppColor.onAccentHigh];
    } else if (status == 'PDF' || status == 'normal' || status == 'PoV Score' || status == 'Berlangsung' || status == 'Sedang' || status =='Total Lokasi' || status == 'Dokumen' || status == 'Total Kunjungan' || status == 'Proses') {

...
class ParsingColor{
  static List<Color> cekColor(String status) {
    ...
    } else if (status == 'PDF' || status == 'normal' || status == 'PoV Score' || status == 'Ongoing' || status == 'Berlangsung' || status == 'Sedang' || status =='Total Lokasi' || status == 'Dokumen' || status == 'Total Kunjungan' || status == 'Proses') {
